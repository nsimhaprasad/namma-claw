...
import { clearSessionCookie, hashPassword, requireAuth, setSessionCookie, signSession, verifyPassword, } from "./auth.js";
import { decryptSecret, encryptSecret } from "./crypto.js";
import { page, escapeHtml } from "./views.js";
import { OpenClawWsClient } from "./openclaw/ws-client.js";
import { docker, createOpenClawInstance, recreateOpenClawContainer, updateOpenClawConfig, stopOpenClawContainer, startOpenClawContainer, deleteOpenClawInstance, clearWhatsAppSession, } from "./docker/openclaw.js";
const app = express();
...
const SecretKeyAllowlist = new Set([
    "OPENAI_API_KEY",
    "ANTHROPIC_API_KEY",
...
async function getInstanceBySlug(slug) {
    const rows = await query("select * from instances where slug=$1 limit 1", [slug]);
    return rows[0] ?? null;
}
async function userHasInstanceAccess(userId, instanceId) {
    const rows = await query("select true as ok from instance_members where instance_id=$1 and user_id=$2 limit 1", [instanceId, userId]);
    return Boolean(rows[0]?.ok);
}
async function isInstanceOwner(userId, instanceId) {
    const rows = await query("select true as ok from instance_members where instance_id=$1 and user_id=$2 and role='owner' limit 1", [instanceId, userId]);
    return Boolean(rows[0]?.ok);
}
async function getInstanceSecrets(instanceId) {
    const rows = await query("select key, ciphertext from instance_secrets where instance_id=$1", [instanceId]);
    const out = {};
    for (const r of rows) {
...
async function upsertInstanceSecret(instanceId, key, value) {
...
app.post("/instances", requireAuth, async (req, res) => {
  ...
  `, [instanceId, session.userId, slug.data, "creating", containerName, volumeName]);
    await query(`insert into instance_members (instance_id, user_id, role) values ($1,$2,$3)`, [instanceId, session.userId, "owner"]);
...
app.get("/i/:slug", requireAuth, async (req, res) => {
    ...
    res.send(page(`Instance ${inst.slug}`, `
            ...
            <form method="post" action="/i/${encodeURIComponent(inst.slug)}/model">
              <label>
                ...
                <select name="defaultModel">
                    ...
                    ${SupportedModels.map(m => `<option value="${m.id}" ${inst.default_model === m.id ? "selected" : ""}>${m.label}</option>`).join("")}
                </select>
              </label>
...
app.post("/i/:slug/stop", requireAuth, async (req, res) => {
    ...
    if (!inst)
        return res.status(404).send("not found");
    if (!(await isInstanceOwner(session.userId, inst.id)))
...
app.post("/i/:slug/secrets", requireAuth, async (req, res) => {
    ...
    if (!SecretKeyAllowlist.has(key))
        return res.status(400).send("unsupported key");
    if (!value.trim())
...
async function rebootInstance(inst) {
    const secrets = await getInstanceSecrets(inst.id);
...
async function withOpenClawClient(inst, fn) {
    const secrets = await getInstanceSecrets(inst.id);
...
app.post("/i/:slug/config", requireAuth, async (req, res) => {
    ...
    await query("update instances set system_prompt=$1, plugins_config=$2, updated_at=now() where id=$3", [systemPrompt || null, JSON.stringify(plugins), inst.id]);
...
app.post("/i/:slug/whatsapp/qr/start", requireAuth, async (req, res) => {
    ...
    if (alreadyConnected) {
        res.send(page(`WhatsApp Connected - ${inst.slug}`, `
        <header class="container" style="padding-bottom: 0; border: none; margin-bottom: 1rem;">
          <nav aria-label="breadcrumb">
              <ul>
              <li><a href="/instances">Instances</a></li>
    ...
    const qr = out?.qrDataUrl ?? null;
    res.send(page(`WhatsApp QR - ${inst.slug}`, `
      <header class="container" style="padding-bottom: 0; border: none; margin-bottom: 1rem;">
        <nav aria-label="breadcrumb">
            <ul>
            <li><a href="/instances">Instances</a></li>
...
app.get("/i/:slug/openclaw", requireAuth, async (req, res) => {
    ...
    if (!(await userHasInstanceAccess(session.userId, inst.id)))
        return res.status(403).send("forbidden");
    if (!inst.power_user_enabled)
        return res.status(400).send("dashboard disabled");
    const secrets = await getInstanceSecrets(inst.id);
    const token = secrets.OPENCLAW_GATEWAY_TOKEN;
    if (!token)
        return res.status(500).send("missing gateway token");
    const dashUrl = `${config.baseUrl}/openclaw/${encodeURIComponent(inst.slug)}/?gatewayUrl=${encodeURIComponent(`${config.baseUrl.replace("http", "ws")}/openclaw/${inst.slug}`)}#token=${encodeURIComponent(token)}`;
...
server.on("upgrade", async (req, socket, head) => {
    try {
        ...
        const slug = parts[1];
        const cookies = (req.headers.cookie || "").split(";").reduce((acc, kv) => {
